...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        if User.current != nil {
            setInitialViewControllerForLoggedInUser()
        }
        
        else {
            setInitialViewControllerForLoggedOutUser()
        }
    }

    func setInitialViewControllerForLoggedInUser() {
          let storyboard = UIStoryboard(name: "Main", bundle: nil)  // Replace "Main" with your actual storyboard name
          let loggedInNavController = storyboard.instantiateViewController(withIdentifier: "FeedNavigationController") as! UINavigationController

          window?.rootViewController = loggedInNavController
      }
    
    func setInitialViewControllerForLoggedOutUser() {
        let storyboard = UIStoryboard(name: "Main", bundle: nil)  // Replace "Main" with your actual storyboard name
        let loggedOutNavController = storyboard.instantiateViewController(withIdentifier: "LoginNavigationController") as! UINavigationController

        window?.rootViewController = loggedOutNavController
    }
